...
export class HomeworkSubmissionController {
    ...
    @ApiOperation({ summary: 'Topshiriqni yuborish (har qanday autentifikatsiya qilingan)' })
    ...
    @ApiBearerAuth()
    @ApiOperation({ summary: 'Barcha topshirilmalar (ADMIN/SUPERADMIN — hammasi, MENTOR — faqat o\'z kurslariniki)' })
    @ApiResponse({ status: 200, description: 'Ro\'yxat' })
    findAll(@Query() query: SubmissionQueryDto, @GetCurrentUser() user: JwtPayload) {
    ...
    findOne(@Param('id', ParseIntPipe) id: number, @GetCurrentUser() user: JwtPayload) {
    ...
    @ApiParam({ name: 'id', type: Number })
    @ApiOperation({ summary: 'Topshirilmani tekshirish: APPROVED / REJECTED (ADMIN, SUPERADMIN, MENTOR — o\'z kursi)' })
    ...
    review(@Param('id', ParseIntPipe) id: number, @Body() dto: ReviewSubmissionDto, @GetCurrentUser() user: JwtPayload) {
