        ...
        select: {
            ...
            lesson: {
                select: {
                    id: true,
                    name: true,
                    group: { select: { id: true, name: true, course: { select: { id: true, name: true, mentorId: true } } } },
                },
            },
        },
    },
...
export class HomeworkSubmissionService {
    ...
    async submit(dto: CreateSubmissionDto, user: JwtPayload) {
        ...
        if (existing) throw new ConflictException('Siz bu topshiriqni allaqachon yuborgansiz');

        return this.prisma.homeworkSubmission.create({
            data: { homeworkId: dto.homeworkId, userId: user.sub, file: dto.file, text: dto.text },
    ...
    async findMy(user: JwtPayload) {
        return this.prisma.homeworkSubmission.findMany({
            ...
            include: {
                homework: { select: { id: true, task: true, lesson: { select: { id: true, name: true } } } },
            },
            orderBy: { createdAt: 'desc' },
        });
    }
    ...
    async findOne(id: number, user: JwtPayload) {
        ...
        if (user.role === UserRole.MENTOR) {
            ...
            if (mentorId !== user.sub) throw new ForbiddenException('Bu kurs sizniki emas');
        }
    ...
    async review(id: number, dto: ReviewSubmissionDto, user: JwtPayload) {
        const submission = await this.prisma.homeworkSubmission.findUnique({
            ...
            include: {
                homework: {
                    include: {
                        lesson: { include: { group: { include: { course: { select: { mentorId: true } } } } } },
                    },
                },
            },
        });
        ...
        return this.prisma.homeworkSubmission.update({
            where: { id },
            data: { status: dto.status, reason: dto.reason, checkedBy: user.sub, updatedAt: new Date() },
            include: submissionInclude,
        });
    }
}
