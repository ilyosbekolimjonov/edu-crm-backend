...
export class GroupService {
    ...
    private async assertStudentAvailability(params: {
        ...
        const { userId, weekDays, startTime, durationMinutes, excludeGroupId } = params;
        ...
        for (const item of studentGroups) {
            ...
            if (hasTimeOverlap(candidateStart, candidateEnd, existingStart, existingEnd)) {
                throw new ConflictException(`Talaba vaqtida to'qnashuv bor: ${existing.name}`);
            }
        }
    }
    ...
    private async assertScheduleAvailability(params: {
        ...
        const { roomId, mentorIds, weekDays, startTime, durationMinutes, excludeGroupId } = params;
        const candidateStart = timeToMinutes(startTime);
        const candidateEnd = candidateStart + durationMinutes;
        ...
        for (const group of activeGroups) {
            ...
            if (!hasTimeOverlap(candidateStart, candidateEnd, groupStart, groupEnd)) continue;

            if (group.roomId === roomId) {
                throw new ConflictException(`Xona band: ${group.name} bilan vaqt to'qnashuvi mavjud`);
            }

            const groupMentorIds = new Set([group.mentorId, ...group.mentorAssignments.map((item) => item.mentorId)]);
            const hasMentorConflict = mentorIds.some((id) => groupMentorIds.has(id));

            if (hasMentorConflict) {
                throw new ConflictException(`O'qituvchi band: ${group.name} bilan vaqt to'qnashuvi mavjud`);
            }
        }
    }

    async create(dto: CreateGroupDto) {
        const mentorIds = Array.from(new Set([dto.mentorId, ...(dto.mentorIds ?? [])].map(Number))).filter((id) => Number.isFinite(id));
        const mentors = await this.prisma.user.findMany({
            where: { id: { in: mentorIds } },
            select: { id: true, role: true },
        });
        if (mentors.length !== mentorIds.length || mentors.some((m) => m.role !== UserRole.MENTOR)) {
            throw new BadRequestException('mentorId/mentorIds ichida MENTOR bo\'lmagan yoki topilmagan user bor');
        }
        ...
        const studentIds = Array.from(new Set((dto.studentIds ?? []).map(Number))).filter((id) => Number.isFinite(id));

        if (studentIds.length > room.capacity) {
            throw new ConflictException(`Xona sig'imi yetarli emas (sig'im: ${room.capacity})`);
        }
        ...
        if (studentIds.length > 0) {
            ...
            if (students.length !== studentIds.length || students.some((s) => s.role !== 'STUDENT')) {
                throw new BadRequestException('studentIds ichida STUDENT bo\'lmagan yoki topilmagan user bor');
            }
        ...
        return this.prisma.$transaction(async (tx) => {
            ...
            if (studentIds.length > 0) {
                await tx.studentGroup.createMany({
                    data: studentIds.map((userId) => ({ userId, groupId: createdGroup.id })),
                    skipDuplicates: true,
                });
            }
            ...
            if (mentorIds.length > 0) {
                await tx.groupMentor.createMany({
                    data: mentorIds.map((mentorId) => ({ mentorId, groupId: createdGroup.id })),
                    skipDuplicates: true,
                });
            }
    ...
    async update(id: number, dto: UpdateGroupDto) {
        ...
        if (requestedMentorIds.length > 0) {
            ...
            if (mentors.length !== requestedMentorIds.length || mentors.some((m) => m.role !== UserRole.MENTOR)) {
                throw new BadRequestException('mentorId/mentorIds ichida MENTOR bo\'lmagan yoki topilmagan user bor');
            }
        }
        ...
        const nextMentorIds = requestedMentorIds.length > 0
            ? requestedMentorIds
        ...
        if (dto.studentIds) {
            const desiredStudentIds = Array.from(new Set(dto.studentIds.map(Number))).filter((n) => Number.isFinite(n));
            const students = await this.prisma.user.findMany({
                where: { id: { in: desiredStudentIds } },
                select: { id: true, role: true },
            });
            if (students.length !== desiredStudentIds.length || students.some((s) => s.role !== 'STUDENT')) {
                throw new BadRequestException('studentIds ichida STUDENT bo\'lmagan yoki topilmagan user bor');
            }

            const room = await this.prisma.room.findUnique({ where: { id: updated.roomId } });
            ...
            const toAdd = desiredStudentIds.filter((userId) => !existingIds.has(userId));
    ...
    async setAttendance(groupId: number, dto: SetAttendanceDto) {
        ...
        if (!membership) throw new NotFoundException('Talaba bu guruhga biriktirilmagan');
