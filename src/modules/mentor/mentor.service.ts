...
export class MentorService {
    ...
    async getCourseStudents(userId: number, courseId: number) {
        ...
        if (!course) {
            throw new NotFoundException('Kurs topilmadi yoki bu kurs sizga tegishli emas');
        }
    ...
    async reviewHomework(userId: number, submissionId: number, dto: ReviewHomeworkDto) {
        ...
        if (!submission) {
            throw new NotFoundException('Topshiriq topilmadi yoki bu topshiriq sizning kursингizga tegishli emas');
        }
        ...
        return { message: `Topshiriq ${dto.status === 'APPROVED' ? 'qabul qilindi' : 'rad etildi'}` };
    }
    ...
    async answerQuestion(userId: number, questionId: number, dto: AnswerQuestionDto) {
        ...
        if (!question) {
            throw new NotFoundException('Savol topilmadi yoki bu savol sizning kursингizga tegishli emas');
        }
    ...
    async getCourseAnalytics(userId: number, courseId: number) {
        ...
        const [ studentsCount, ratings, totalLessons, completedViews, homeworksSubmitted, pendingHomeworks, unansweredQuestions, ] = await Promise.all([
            this.prisma.purchasedCourse.count({ where: { courseId } }),
            this.prisma.rating.findMany({ where: { courseId }, select: { rate: true } }),
            this.prisma.lesson.count({ where: { group: { courseId } } }),
            this.prisma.lessonView.count({ where: { lesson: { group: { courseId } }, view: true } }),
        ...
        const averageRating =
            ratings.length > 0
                ? parseFloat(
                    (ratings.reduce((s, r) => s + r.rate, 0) / ratings.length).toFixed(1),
                )
                : 0;
