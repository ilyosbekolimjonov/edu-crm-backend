...
export class AuthService {
    ...
    async register(dto: RegisterDto, creatorRole: UserRole): Promise<{ message: string; userId: number }> {
    ...
    async login(dto: LoginDto): Promise<Tokens> {
        const user = await this.prisma.user.findFirst({
            where: { OR: [{ email: dto.login }, { username: dto.login }, { phone: dto.login }] },
        });
    ...
    async updateUser(id: number, dto: UpdateUserDto, currentUserId: number, currentUserRole: UserRole) {
        const user = await this.prisma.user.findUnique({ where: { id }, include: { Mentor: true } });
        if (!user) {
            throw new NotFoundException('Foydalanuvchi topilmadi');
        }

        if (currentUserRole === UserRole.ADMIN && user.role === UserRole.SUPERADMIN) {
            throw new ForbiddenException('ADMIN SUPERADMIN foydalanuvchini tahrirlay olmaydi');
        }

        if (currentUserRole === UserRole.ADMIN && dto.role === UserRole.SUPERADMIN) {
            throw new ForbiddenException('ADMIN foydalanuvchini SUPERADMIN roliga o\'zgartira olmaydi');
        }
        ...
        if (mentorFieldsProvided && targetRole !== UserRole.MENTOR) {
            throw new BadRequestException('Mentor maydonlari faqat MENTOR uchun ruxsat etiladi');
        }
    ...
    async removeUser(id: number, currentUserId: number, currentUserRole: UserRole) {
        if (id === currentUserId) {
        ...
        if (currentUserRole === UserRole.ADMIN && user.role === UserRole.SUPERADMIN) {
            throw new ForbiddenException('ADMIN SUPERADMIN foydalanuvchini o\'chira olmaydi');
        }

        if (user.role === UserRole.MENTOR) {
            const mentorGroupsCount = await this.prisma.group.count({ where: { mentorId: id } });
            if (mentorGroupsCount > 0) {
                throw new ConflictException('Bu o\'qituvchiga bog\'langan guruhlar bor, avval guruhlarni ajrating');
            }
        }
