...
export class LessonService {
    ...
    async findByGroup(lessonGroupId: number, userId: number) {
        ...
        return lessons.map(l => ({ ...l, viewed: l.views[0]?.view ?? false, views: undefined }));
    }
    ...
    async findOne(id: string, userId: number) {
        ...
        return { ...lesson, viewed: lesson.views[0]?.view ?? false, views: undefined };
    }

    async update(id: string, dto: UpdateLessonDto, user: JwtPayload) {
        const lesson = await this.prisma.lesson.findUnique({
            where: { id },
            include: { group: { include: { course: { select: { mentorId: true } } } } },
        });
        if (!lesson) throw new NotFoundException('Dars topilmadi');
        this.checkMentorOwnership(lesson.group.course.mentorId, user);

        return this.prisma.lesson.update({ where: { id }, data: { ...dto, updatedAt: new Date() } });
    }
    ...
    async addFile(id: string, dto: AddLessonFileDto, user: JwtPayload) {
        ...
        return this.prisma.lessonFile.create({ data: { file: dto.file, note: dto.note, lessonId: id } });
    }

    async removeFile(id: string, fileId: number, user: JwtPayload) {
        const file = await this.prisma.lessonFile.findUnique({
            where: { id: fileId },
            include: { lesson: { include: { group: { include: { course: { select: { mentorId: true } } } } } } },
        });
        if (!file || file.lessonId !== id) throw new NotFoundException('Fayl topilmadi');
